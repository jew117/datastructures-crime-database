...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Only allow edit if user is admin OR if user owns the observation
    if (!$is_admin && $observation['user_id'] != $user_id) {
        echo json_encode(['success' => false, 'message' => 'You do not have permission to edit this observation']);
        exit();
    }
    
    // Update the observation
    $sql = "UPDATE incident_observations SET observation = ?, updated_at = NOW() WHERE id = ?";
    ...
    $stmt->bind_param("si", $observation_text, $observation_id);
